...
npm run catalog:promote -- --word stone --cluster nature --target practice
...
If a host does not provide `/usr/share/dict/words`, the suggestion generator falls back to the embedding dataset itself so production builds still succeed.

To rotate a fresh suggestion into the live seed:

1. Run `npm run catalog:suggest`
2. Pick a word from `catalog/wordCatalog.suggestions.json`
3. Promote it with `npm run catalog:promote -- --word <word> --cluster <cluster> --target <daily|practice>`
4. Run `npm run catalog:refresh`
